...
namespace Doozy.Editor.Bindy.ScriptableObjects
{
    [Serializable]
    public class BindIdDatabase : SingletonEditorScriptableObject<BindIdDatabase>, IUpdateCallback
    {
       #region IUpdateCallback
        ...
        [SerializeField] private BindIdDataGroup Database;
        public BindIdDataGroup database => Database ??= new BindIdDataGroup();

        [RestoreData(nameof(BindIdDatabase))]
        public static BindIdDatabase RestoreData() =>
            instance;
        
        public BindIdDatabase() => Database = new BindIdDataGroup();

        public (bool, string) CanImportRoamingDatabases(List<BindIdRoamingDatabase> roamingDatabases)
        {
            roamingDatabases ??= new List<BindIdRoamingDatabase>();         //avoid null
        ...
        {
            var roamingDatabases = databases.Cast<BindIdRoamingDatabase>().ToList();
            bool canImport;
            string message;
            ...
            foreach (BindIdRoamingDatabase roamingDatabase in roamingDatabases)
            {
                counter++;
        ...
        {
            string[] guids = AssetDatabase.FindAssets($"t:{nameof(BindIdRoamingDatabase)}");
            if (guids == null || guids.Length == 0)
            {
                EditorUtility.DisplayDialog("Import Databases", $"No {nameof(BindIdRoamingDatabase)} databases were found", "Ok");
                return false;
            }
            ...
            {
                counter++;
                BindIdRoamingDatabase asset = AssetDatabase.LoadAssetAtPath<BindIdRoamingDatabase>(AssetDatabase.GUIDToAssetPath(guid));
        ...
        public bool ExportRoamingDatabase(string databaseName, List<string> selectedCategories)
        {
            ...
            BindIdRoamingDatabase roamingDatabase = CreateInstance<BindIdRoamingDatabase>();
            roamingDatabase.databaseName = databaseName;
            roamingDatabase.database.ClearDatabase();
            string savePath = EditorUtility.SaveFilePanelInProject("Export Roaming Database", $"{databaseName}_{nameof(BindIdRoamingDatabase)}", "asset", "Save to");
